...
loadResource :: ResourceConfig -> IO [Nxt.Types.Texture]
loadResource (LoadTexture fp) = do 
    dataPath <- getDataDir
    (: []) <$> loadTexture (dataPath ++ fp)
loadResource (CycleTexture fp frames frameTime) = do 
    dataPath <- getDataDir
    cycleTextures (dataPath ++ fp) frames frameTime
loadResource (RepeatTextures fp frames startRepeat endRepeat nRepeats lastFrame frameTime) = do 
    dataPath <- getDataDir
    repeatTexturesN (dataPath ++ fp) frames startRepeat endRepeat nRepeats lastFrame frameTime
loadResource (CycleLastFrame fp frames lastFrame frameTime) = do 
    dataPath <- getDataDir
    cycleTextures2 (dataPath ++ fp) frames lastFrame frameTime

loadResources :: [ResourceConfig] -> IO (M.Map ResourceConfig [Nxt.Types.Texture])
loadResources rcs = do
    ts <- mapM loadResource rcs
    let kvList = zip rcs ts
    return (M.fromList kvList)