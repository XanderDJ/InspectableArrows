...
class PatternedArrow a => TexturedArrow a where
...
ifA :: TexturedArrow a => a b (Either c d) -> a d e -> a c e -> a b e
ifA a b1 b2 = a >>> b2 ||| b1

notA :: TexturedArrow a => a (Either c d) (Either d c)
notA = arr swap
  where
    swap (Right x) = Left x
    swap (Left x) = Right x
...
class TexturedArrow a => MapA a where
...
instance MapA (ConstA (S.Set ResourceIdentifier)) where
  mapA (ConstA rs) (ConstA rs') = ConstA (rs <> rs')

instance Monoid a => ArrowApply (ConstA a) where  
  app = ConstA mempty

instance Monoid a => ArrowLoop (ConstA a) where
  loop  (ConstA a)= ConstA a 
